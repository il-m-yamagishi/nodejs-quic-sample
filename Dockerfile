...
RUN ARCH= && dpkgArch="$(dpkg --print-architecture)" \
    ...
    && set -ex \
    # libatomic1 for arm
    && apt-get update && apt-get install -y ca-certificates curl wget gnupg dirmngr xz-utils libatomic1 python3 g++ make python3-distutils --no-install-recommends \
...
RUN rm -rf node-v${NODE_VERSION}.tar.gz node-v${NODE_VERSION}
